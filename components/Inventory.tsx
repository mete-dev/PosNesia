...
const ShelfModal: React.FC<{
    ...
    const handleSubmit = (e: React.FormEvent) => {
        e.preventDefault();
        const shelfData = { code, description, shelvingCount, locationType, locationId };
        if (!locationId) {
            alert("Harap pilih lokasi spesifik.");
            return;
        }
        if (existingShelf) {
            ...
            <div className="bg-white dark:bg-gray-800 rounded-lg shadow-xl p-8 w-full max-w-2xl">
                ...
                <form onSubmit={handleSubmit} className="space-y-4">
                    <div className="grid grid-cols-2 gap-4">
                        <Input value={code} onChange={e => setCode(e.target.value)} placeholder="Kode Rak (e.g., A-01)" required />
                    ...
                    <Input value={description} onChange={e => setDescription(e.target.value)} placeholder="Deskripsi (e.g., Lorong depan)" />
                     <div className="grid grid-cols-3 gap-4 border-t pt-4 dark:border-gray-600">
                        <div>
                            <Label>Tipe Lokasi</Label>
                            <Select value={locationType} onChange={e => { setLocationType(e.target.value as any); setSelectedLocationTypeId(''); setLocationId(''); }}>
                                <option value="warehouse">Gudang</option>
                                <option value="branch">Toko</option>
                            </Select>
                        </div>
                        <div>
                            <Label>Kategori {locationType === 'warehouse' ? 'Gudang' : 'Toko'}</Label>
                            <Select value={selectedLocationTypeId} onChange={e => { setSelectedLocationTypeId(e.target.value); setLocationId(''); }} required>
                        ...
                        <div>
                            ...
                            <Select value={locationId} onChange={e => setLocationId(e.target.value)} required disabled={!selectedLocationTypeId}>
                    ...
                    <tbody>
                        {shelves.map(shelf => (
                            <tr key={shelf.id} className="bg-white dark:bg-gray-800 border-b dark:border-gray-700">
                                ...
                                <td className="px-6 py-4">{locationMap.get(shelf.locationId) || shelf.locationId}</td>
